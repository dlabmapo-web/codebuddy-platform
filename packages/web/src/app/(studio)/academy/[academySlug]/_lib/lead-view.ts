...
export function exerciseHref(
  ...
  return `${routes.academy(academySlug)}/content/courses/${courseId}#exercise-${materialId}`;
}

export function courseHref(academySlug: string, courseId: string): string {
  return `${routes.academy(academySlug)}/content/courses/${courseId}`;
}

/** One class's settings, where its teacher and its courses are assigned. */
export function classHref(academySlug: string, classId: string): string {
  return `${routes.academy(academySlug)}/classes/${classId}`;
}
