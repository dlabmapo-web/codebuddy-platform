...
function StatCard({
  ...
  return (
    <div className="bg-white rounded-xl p-4 flex items-start gap-3" style={{ border: '1px solid #E5E8EC' }}>
...
function ChartCard({
  ...
  return (
    <section className="bg-white rounded-xl p-5 min-w-0" style={{ border: '1px solid #E5E8EC' }}>
      <div className="mb-3">
...
export function TeacherAnalyticsDashboard() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/teacher/dashboard?range=${range}`, { signal: controller.signal })
      .then(async (response) => {
    ...
    <section className="flex flex-col gap-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p style={{ fontSize: '12px', color: '#8A8F98', marginTop: 2 }}>제출 기록을 기반으로 학습 흐름과 지원이 필요한 학생을 확인하세요.</p>
        </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            ...
            <ChartCard title="해결이 어려운 문제" description="학생 해결률이 낮은 문제부터 표시합니다." empty={data.problemPerformance.length === 0}>
