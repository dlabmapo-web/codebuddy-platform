...
function fixture(text: Y.Text, offset = 0) {
  ...
  const editor = {
    ...
    getScrolledVisiblePosition: (p: { column: number }) => ({ left: 40 + p.column * 8, top: 20 }),
...
describe('code pointer anchors', () => {
  it('tracks an emoji boundary through concurrent insertion and projects using receiver geometry', () => {
    ...
    const pointer = captureCodePointer(sender.element, { clientX: 1, clientY: 2 }, draftId)!;
    sender.unregister();
    ...
    receiver.editor.getScrolledVisiblePosition = () => ({ left: 100, top: 80 });
  ...
  it('suppresses wrong identity, invalid anchors, non-text and off-screen targets', () => {
    ...
    const pointer = captureCodePointer(peer.element, { clientX: 0, clientY: 0 }, draftId)!;
    ...
    expect(captureCodePointer(peer.element, { clientX: 0, clientY: 0 }, draftId)).toBeNull();
    peer.unregister();
  });
});
