...
/**
 * The parts every academy overview is built from.
 *
 * They live here, above both the teacher's overview and the manager's control
 * tower, because the two pages have to read as one product. A panel header
 * always carries its own denominator, a measurement is always in tabular
 * figures, a missing number is always an em dash with a spoken label. Repeating
 * those decisions per surface is how a product ends up with four slightly
 * different progress bars and three ways of saying "no data" — and a manager
 * comparing their page to a teacher's would be right that they disagree.
 *
 * Nothing here reads a translation namespace. That is the line between this
 * file and the two beside it: a primitive that fetched its own copy could only
 * ever belong to one page.
 *
 * ## Why the pages are coloured
 *
 * Seven sections, seven questions, and — before this — seven identical white
 * cards. A reader scrolling past the third one had nothing to tell them which
 * question they had landed on, and these pages exist to be read quickly.
 *
 ...
 * what is not ready, red asks what is blocking them. It appears as a rail
 * across the top of the card, an icon in the header, and the section's own
 * accents — the body of every panel stays a plain reading surface.
 *
