...
function StatCard({
  ...
  return (
    <div className="bg-card rounded-xl p-4 flex items-start gap-3 min-w-0" style={{ border: '1px solid var(--color-border)' }}>
      ...
      <div className="min-w-0">
        <p style={{ fontSize: '12px', color: 'var(--color-sub)', marginBottom: 3 }}>{label}</p>
        <div className="flex items-baseline gap-1">
          <strong style={{ fontSize: '22px', lineHeight: 1.1, color: 'var(--color-ink)' }}>{formatNumber(value)}</strong>
          <span style={{ fontSize: '12px', color: 'var(--color-sub)' }}>{unit}</span>
        </div>
...
function ChartCard({
  ...
  return (
    <section className="bg-card rounded-xl p-5 min-w-0 overflow-hidden" style={{ border: '1px solid var(--color-border)' }}>
      <div className="mb-3">
        <h3 style={{ fontSize: '14px', fontWeight: 700, color: 'var(--color-ink)' }}>{title}</h3>
        <p style={{ fontSize: '12px', color: 'var(--color-sub)', marginTop: 2 }}>{description}</p>
      </div>
      {empty ? (
        <div className="flex items-center justify-center rounded-xl" style={{ height: 260, backgroundColor: 'var(--color-muted)', color: '#BCC0C7', fontSize: '13px' }}>
  ...
  return (
    <label className="flex flex-col gap-1 min-w-0">
      ...
      <select
        ...
        style={{ height: 34, border: '1px solid var(--color-border)', fontSize: '12px', color: 'var(--color-ink)', minWidth: 120, maxWidth: 180 }}
      >
    ...
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={index} className="animate-pulse rounded-xl bg-card" style={{ height: 112, border: '1px solid var(--color-border)' }} />
        ))}
      </div>
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={index} className="bg-card rounded-xl p-5" style={{ border: '1px solid var(--color-border)' }}>
  ...
  return (
    <section className="flex flex-col gap-4 min-w-0">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 style={{ fontSize: '17px', fontWeight: 700, color: 'var(--color-ink)' }}>학습 분석</h2>
          <p style={{ fontSize: '12px', color: 'var(--color-sub)', marginTop: 2 }}>과목·단계·챕터 단위로 학습 흐름을 확인하세요.</p>
        </div>
        <div className="flex flex-wrap items-end gap-3">
          ...
          />
          <div className="flex items-center gap-1 rounded-lg bg-card p-1" style={{ border: '1px solid var(--color-border)' }}>
            {RANGE_OPTIONS.map((option) => (
              <button
                ...
                style={{
                  ...
                  fontSize: '12px',
                  fontWeight: 600,
                  backgroundColor: range === option.value ? 'var(--color-primary)' : 'transparent',
                  color: range === option.value ? 'white' : 'var(--color-sub)',
                }}
              >
                {option.label}
              </button>
      ...
      {loading ? (
        ...
        <div className="bg-card rounded-xl flex flex-col items-center justify-center gap-3 py-10" style={{ border: '1px solid var(--color-border)' }}>
          <AlertTriangle size={24} style={{ color: '#D97706' }} />
          <p style={{ fontSize: '13px', color: 'var(--color-sub)' }}>{error}</p>
          <button
            onClick={() => setReloadKey((key) => key + 1)}
            className="flex items-center gap-1.5 px-3 rounded-lg"
            style={{ height: 34, border: '1px solid var(--color-border)', fontSize: '12px', fontWeight: 600, color: 'var(--color-primary)' }}
          >
        ...
        <>
          ...
          <section className="bg-card rounded-xl overflow-hidden" style={{ border: '1px solid var(--color-border)' }}>
            <div className="flex items-center gap-2 px-5 py-4" style={{ borderBottom: '1px solid var(--color-border)' }}>
              <AlertTriangle size={15} style={{ color: '#D97706' }} />
              <div>
                <h3 style={{ fontSize: '14px', fontWeight: 700, color: 'var(--color-ink)' }}>지원이 필요한 학생</h3>
                <p style={{ fontSize: '12px', color: 'var(--color-sub)', marginTop: 1 }}>오답이 2회 이상이고 해결률이 60% 미만인 학생입니다.</p>
              </div>
            </div>
            {data.studentsNeedingHelp.length === 0 ? (
              <div className="px-5 py-8 text-center" style={{ fontSize: '13px', color: 'var(--color-sub)' }}>
                  ...
                  <div key={student.studentId} className="flex items-center gap-4 px-5 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--color-ink)' }}>{student.name}</span>
                        <span style={{ fontSize: '11px', color: '#BCC0C7' }}>@{student.username}</span>
                      </div>
                      <p style={{ fontSize: '11px', color: 'var(--color-sub)', marginTop: 2 }}>
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <p style={{ fontSize: '11px', color: 'var(--color-sub)', marginTop: 2 }}>해결률 {student.solveRate}%</p>
                    </div>
                  </div>
                ))}
              </div>
