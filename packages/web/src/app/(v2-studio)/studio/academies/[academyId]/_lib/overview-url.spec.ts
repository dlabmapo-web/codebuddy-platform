...
describe('solutionStatusPath', () => {
  it('opens one student in the By-student lens', () => {
    expect(
      solutionStatusPath({ academyId, classId: classA, membershipId }),
    ).toBe(
      `/studio/academies/${academyId}/teach/classes/${classA}/progress` +
  ...
  it('has nowhere authorized to send an academy-wide signal', () => {
    ...
    expect(solutionStatusPath({ academyId, classId: null })).toBeNull();
  });
});
...
describe('studentAnalyticsPath', () => {
  ...
  it('leaves the default period out of the address', () => {
    expect(
      studentAnalyticsPath({ academyId, query: defaultOverviewQuery }),
    ).toBe(`/studio/academies/${academyId}/teach/students`);
  });
