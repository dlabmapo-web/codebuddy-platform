...
export async function GET() {
  ...
  const { data, error } = await db
    .from('problems')
    .select('id, problem_no, title, difficulty, is_published, created_at')
...
export async function POST(req: Request) {
  ...
  const { title, difficulty, description, input_format, output_format, constraint_text, starter_code, time_limit_ms, memory_limit_mb, is_published, test_cases, hints } = body as {
    title: string;
    difficulty: string;
    description: string;
    input_format?: string;
