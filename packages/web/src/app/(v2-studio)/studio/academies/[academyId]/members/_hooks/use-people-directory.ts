...
export function usePeopleDirectoryState(academyId: string) {
  ...
  const path = peoplePath(academyId, query);
  React.useEffect(() => {
...
export function peoplePath(academyId: string, query: PeopleQuery): string {
  const search = serializePeopleQuery(query);
  const base = `/studio/academies/${academyId}/members`;
