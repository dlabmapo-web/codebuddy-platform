...
export function recordsPath(academyId: string, query: RecordsQuery): string {
  const search = serializeRecordsQuery(query);
  const base = `/studio/academies/${academyId}/learn/records`;
...
export function safeReturnTo(
  ...
  const root = `/studio/academies/${academyId}/learn/records`;
  ...
  return recordsPath(academyId, parseRecordsQuery(new URLSearchParams(search)));
}
