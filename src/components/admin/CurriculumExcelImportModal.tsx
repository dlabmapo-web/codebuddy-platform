...
function validate(rows: ImportRow[], testCaseKeys: Set<string>, hintKeys: Set<string>) {
  ...
  rows.forEach((row, index) => {
    ...
    row.test_cases.forEach((testCase, testIndex) => {
      if (row.problem.input_format && testCase.input === '') {
        errors.push(`${label}: 테스트케이스 ${testIndex + 1}의 표준 입력값이 없습니다.`);
      }
      ...
      >
        <header className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid var(--color-border)' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 style={{ fontSize: '17px', fontWeight: 800, color: 'var(--color-ink)' }}>엑셀 일괄 등록</h2>
              <p style={{ fontSize: '12px', color: 'var(--color-sub)', marginTop: 2 }}>과목부터 문제·테스트케이스까지 한 번에 등록합니다.</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="mb-5 grid grid-cols-1 gap-3 md:grid-cols-[1fr_auto]">
            <div className="rounded-xl bg-[var(--color-muted)] px-4 py-3" style={{ border: '1px solid var(--color-border)' }}>
              <p style={{ fontSize: '13px', fontWeight: 700, color: 'var(--color-ink)' }}>등록 방법</p>
              <ol className="mt-2 list-decimal space-y-1 pl-4" style={{ fontSize: '12px', lineHeight: 1.55, color: 'var(--color-sub)' }}>
          ...
          <button
            ...
            style={{ borderColor: dragging ? 'var(--color-primary)' : '#C9CED6', backgroundColor: dragging ? 'var(--tint-soft)' : 'var(--color-muted)' }}
          >
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-card text-primary shadow-sm">
              <Upload size={21} />
            </div>
            <p className="mt-3" style={{ fontSize: '14px', fontWeight: 700, color: 'var(--color-ink)' }}>
              {parsing ? '엑셀 파일을 읽는 중...' : fileName || '엑셀 파일을 선택하거나 여기에 놓으세요'}
            </p>
            <p className="mt-1" style={{ fontSize: '12px', color: 'var(--color-sub)' }}>.xlsx 또는 .xls · 최대 200개 문제</p>
          </button>
          ...
          {rows.length > 0 && (
            <div className="mt-5">
              <div className="mb-3 flex flex-wrap items-center gap-2">
                ...
                <span className="rounded-lg bg-surface px-3 py-1.5" style={{ fontSize: '12px', color: 'var(--color-sub)' }}>
                  ...
                  <thead>
                    <tr className="bg-[var(--color-muted)]">
                      {['문제키', '과목', '단계', '챕터', '문제', '난이도', '테스트'].map((header) => (
                        <th key={header} className="px-3 py-2.5 text-left" style={{ fontSize: '11px', fontWeight: 700, color: 'var(--color-sub)', borderBottom: '1px solid var(--color-border)' }}>{header}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {rows.slice(0, 10).map((row, index) => (
                      <tr key={`${row.key}-${index}`} style={{ borderBottom: index < Math.min(rows.length, 10) - 1 ? '1px solid var(--color-muted)' : 'none' }}>
                        <td className="px-3 py-2.5" style={{ fontSize: '12px', fontFamily: 'monospace', color: 'var(--color-sub)' }}>{row.key}</td>
                        <td className="px-3 py-2.5" style={{ fontSize: '12px', color: 'var(--color-ink)' }}>{row.subject.title}</td>
                        <td className="px-3 py-2.5" style={{ fontSize: '12px', color: 'var(--color-ink)' }}>{row.stage.title}</td>
                        <td className="px-3 py-2.5" style={{ fontSize: '12px', color: 'var(--color-ink)' }}>{row.chapter.title}</td>
                        <td className="px-3 py-2.5" style={{ fontSize: '12px', fontWeight: 600, color: 'var(--color-ink)' }}>{row.problem.title}</td>
                        <td className="px-3 py-2.5" style={{ fontSize: '12px', color: 'var(--color-sub)' }}>{row.problem.difficulty}</td>
                        <td className="px-3 py-2.5" style={{ fontSize: '12px', color: 'var(--color-sub)' }}>{row.test_cases.length}개</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
              {rows.length > 10 && <p className="mt-2 text-right" style={{ fontSize: '11px', color: 'var(--color-sub)' }}>외 {rows.length - 10}개 문제</p>}
            </div>
          )}
        </div>

        <footer className="flex items-center justify-between gap-3 px-6 py-4" style={{ borderTop: '1px solid var(--color-border)', backgroundColor: 'var(--color-muted)' }}>
          <p style={{ fontSize: '11px', color: 'var(--color-sub)' }}>
            기존 항목과 번호가 충돌하면 등록하지 않고 오류를 안내합니다.
          </p>
          <div className="flex shrink-0 gap-2">
            <button onClick={onClose} className="rounded-xl px-5" style={{ height: 40, border: '1px solid var(--color-border)', fontSize: '13px', fontWeight: 700, color: 'var(--color-sub)' }}>취소</button>
            <button
              onClick={submit}
              disabled={!ready || importing}
              className="rounded-xl px-5 text-white disabled:opacity-40"
              style={{ height: 40, backgroundColor: 'var(--color-primary)', fontSize: '13px', fontWeight: 700 }}
            >
